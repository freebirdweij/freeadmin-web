    ...
    <div class="head-container">
      ...
      <el-dialog ref="dialog" :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" width="500px">
        <el-form ref="form" :model="form" :rules="rules" size="small" label-width="80px">
          <el-form-item label="调往仓库" prop="storeId">
          ...
          <div v-else-if="inGoods===1">
            <el-form-item ref="counts" label="调拨数量" prop="counts">
              <el-input v-model="form.counts" style="width: 370px;" />
            </el-form-item>
          </div>
          <div v-else>
            <el-form-item ref="counts" label="盘点数量" prop="counts">
              <el-input v-model="form.counts" style="width: 370px;" />
            </el-form-item>
          </div>
        </el-form>
        ...
        <div v-else-if="inGoods===1" slot="footer" class="dialog-footer">
          <el-button type="text" @click="crud.cancelCU">取消</el-button>
          <el-button :loading="crud.status.cu === 2" type="primary" @click="doMovegoods(form)">确认</el-button>
        </div>
        <div v-else slot="footer" class="dialog-footer">
          <el-button type="text" @click="crud.cancelCU">取消</el-button>
          <el-button :loading="crud.status.cu === 2" type="primary" @click="doModifygoods(form)">确认</el-button>
        </div>
      </el-dialog>
      <!--表格渲染-->
...
import crudStoreRemain from '@/api/store/storeModify'
...
export default {
  ...
  cruds() {
    return CRUD({ title: '库存', url: 'api/storeModify', idField: 'remainId', sort: 'remainId,desc', crudMethod: { ...crudStoreRemain }})
  },
  data() {
    return {
      keyword: '',
  ...
  methods: {
    ...
    doMovegoods(data) {
      crudStoreRemain.moveGoods(data).then(() => {
    ...
    doModifygoods(data) {
      crudStoreRemain.modifyGoods(data).then(() => {
        this.crud.status.add = CRUD.STATUS.NORMAL
        this.crud.status.edit = CRUD.STATUS.NORMAL
        this.crud.refresh()
      })
