    ...
    <div class="head-container">
      ...
      <el-dialog ref="dialog" :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" width="500px">
        <el-form ref="form" :model="form" :rules="rules" size="small" label-width="80px">
          ...
          <div v-if="inGoods===0">
            <el-form-item ref="counts" label="数量" prop="counts">
              <el-input v-model="form.counts" style="width: 370px;" />
            </el-form-item>
          </div>
          <div v-else-if="inGoods===1">
            <el-form-item ref="counts" label="领料数量" prop="counts">
              <el-input v-model="form.counts" style="width: 370px;" />
            </el-form-item>
          </div>
          <div v-else>
            <el-form-item ref="counts" label="退料数量" prop="counts">
              <el-input v-model="form.counts" style="width: 370px;" />
            </el-form-item>
          </div>
        </el-form>
        ...
        <div v-else-if="inGoods===1" slot="footer" class="dialog-footer">
          <el-button type="text" @click="crud.cancelCU">取消</el-button>
          <el-button :loading="crud.status.cu === 2" type="primary" @click="doTakegoods(form)">确认</el-button>
        </div>
        <div v-else slot="footer" class="dialog-footer">
          <el-button type="text" @click="crud.cancelCU">取消</el-button>
          <el-button :loading="crud.status.cu === 2" type="primary" @click="doRefundgoods(form)">确认</el-button>
        </div>
      </el-dialog>
      <!--表格渲染-->
...
import crudStoreRemain from '@/api/store/storeSale'
...
export default {
  ...
  cruds() {
    return CRUD({ title: '库存', url: 'api/storeSale', idField: 'remainId', sort: 'remainId,desc', crudMethod: { ...crudStoreRemain }})
  },
  data() {
    return {
      keyword: '',
  ...
  methods: {
    ...
    doTakegoods(data) {
      crudStoreRemain.takeGoods(data).then(() => {
    ...
    doRefundgoods(data) {
      crudStoreRemain.refundGoods(data).then(() => {
        this.crud.status.add = CRUD.STATUS.NORMAL
        this.crud.status.edit = CRUD.STATUS.NORMAL
        this.crud.refresh()
      })
